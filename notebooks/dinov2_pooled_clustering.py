...
# ── Load and pool ─────────────────────────────────────────────────────────────
all_R2_cls = []; all_R2_patch = []; all_depth_cls = []; all_depth_patch = []
all_peak_cls = []; all_peak_patch = []; all_monkey_id = []
t_ms = None

for mi, monkey in enumerate(MONKEYS):
    with open(f'{CACHE_DIR}/time_resolved_perunit_dinov2_{monkey}.pkl','rb') as f:
        res = pkl.load(f)
    R2_cls   = res['r2_all']['cls'].astype(np.float64)    # (12, 90, n_units)
...
depth_cls     = np.concatenate(all_depth_cls,  axis=1)  # (90,N)
depth_patch   = np.concatenate(all_depth_patch,axis=1)
peak_cls      = np.concatenate(all_peak_cls)             # (N,)
...
for u in range(N):
    if peak_cls[u] < 0.05: continue
    # CLS depth curve
    dc = fill_depth(depth_cls[win_t, u])
    ...
    ed = np.nanmean(depth_cls[early_t, u])
    ld = np.nanmean(depth_cls[late_t,  u])
...
for k_use in sorted(set([best_k, 4, 5])):
    ...
    for c in range(k_use):
        ...
        dc_c     = depth_cls[:, unit_idx]
        pk_c     = peak_cls[unit_idx]
    ...
    fig.suptitle(f'DINOv2 CLS Pooled Clusters (k={k_use}, N={n_good}, 5 monkeys)',
    ...
    for ci, cs in enumerate(cluster_stats):
        ...
        for u in cs['unit_idx'][:60]:
            dc = depth_cls[:, u]; valid = np.isfinite(dc)
            if valid.sum() > 5:
    ...
    outpath = f'{FIGDIR}/fig_dinov2_pooled_clusters_k{k_use}.png'
...
for mi, monkey in enumerate(MONKEYS):
    with open(f'{CACHE_DIR}/time_resolved_perunit_dinov2_{monkey}.pkl','rb') as f:
        res = pkl.load(f)
    R2m = res['r2_all']['cls'].astype(np.float64)  # (12,90,n_units)
...
plt.tight_layout()
outpath_mk = f'{FIGDIR}/fig_dinov2_per_monkey.png'
